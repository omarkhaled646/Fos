...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	uint32 currnt_va = virtual_address;
	int numberOfPages = size / PAGE_SIZE;
	for (int i = 0; i < numberOfPages; i++) {
		int ret = pf_add_empty_env_page(e, currnt_va, 0);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	//free active list
	uint32 final_add = virtual_address + (numberOfPages * PAGE_SIZE);
	struct WorkingSetElement* element;
	LIST_FOREACH(element,&(e->ActiveList))
	{
		if (element->virtual_address >= virtual_address && element->virtual_address < final_add)
		{
			...
			{
				struct WorkingSetElement *secondHead = LIST_FIRST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList), secondHead);
				LIST_INSERT_TAIL(&(e->ActiveList), secondHead);
				pt_set_page_permissions(e, secondHead->virtual_address,PERM_PRESENT | PERM_WRITEABLE, 0);
			}
			...
			if (counter == 1024)
			{
				 struct Frame_Info* ptr_frame_info  = get_frame_info(e->env_page_directory,(void*) ptr_page_table, &ptr_page_table);
				 free_frame(ptr_frame_info);
